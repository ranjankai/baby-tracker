...
- [DONE] **27-05-26**: Implemented Telegram Bot Chatbox (`supabase/functions/telegram-bot/index.ts`) on the same bot that sends notifications. Capabilities: natural language Q&A (two-tier Gemini pipeline — Gemma triage → Gemini expert, IST-aware, env context), log export as file attachments (Doctor plain text `.txt` and ChatGPT markdown `.md`), and self-registering webhook. Webhook URL: `https://vyaleoetmmxjsykirfop.supabase.co/functions/v1/telegram-bot`.
